        ...
        <table class="table mt-5 w-auto table-bordered admin-table">
            ...
            @foreach ($ads as $ad)
                <tr>
                    ...
                    <td>{{ $ad->user->name }}</td>
                    <td>
                        <form class="form-group admin-table__column" action="{{ route('adUpdate', $ad['id']) }}" method="post">
                            @csrf
                            @method('PUT')
                            <select name="category_id" class="form-select" @if(Auth::user()->isModerator()) disabled @endif>
                            ...
                            <button class="btn btn-success admin-table__bth-change" type="submit" @if(Auth::user()->isModerator()) disabled @endif>
                                <p>Изменить</p>
                                <svg class="svgAdmin admin-table__bth-svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.com/svgjs" version="1.1" width="512" height="512" x="0" y="0" viewBox="0 0 24 24" style="enable-background:new 0 0 512 512" xml:space="preserve"><g>
                                        <path xmlns="http://www.w3.org/2000/svg" d="M18.656.93,6.464,13.122A4.966,4.966,0,0,0,5,16.657V18a1,1,0,0,0,1,1H7.343a4.966,4.966,0,0,0,3.535-1.464L23.07,5.344a3.125,3.125,0,0,0,0-4.414A3.194,3.194,0,0,0,18.656.93Zm3,3L9.464,16.122A3.02,3.02,0,0,1,7.343,17H7v-.343a3.02,3.02,0,0,1,.878-2.121L20.07,2.344a1.148,1.148,0,0,1,1.586,0A1.123,1.123,0,0,1,21.656,3.93Z" fill="#e5e9ec" data-original="#000000"/>
                                        <path xmlns="http://www.w3.org/2000/svg" d="M23,8.979a1,1,0,0,0-1,1V15H18a3,3,0,0,0-3,3v4H5a3,3,0,0,1-3-3V5A3,3,0,0,1,5,2h9.042a1,1,0,0,0,0-2H5A5.006,5.006,0,0,0,0,5V19a5.006,5.006,0,0,0,5,5H16.343a4.968,4.968,0,0,0,3.536-1.464l2.656-2.658A4.968,4.968,0,0,0,24,16.343V9.979A1,1,0,0,0,23,8.979ZM18.465,21.122a2.975,2.975,0,0,1-1.465.8V18a1,1,0,0,1,1-1h3.925a3.016,3.016,0,0,1-.8,1.464Z" fill="#e5e9ec" data-original="#000000"/></g>
                                </svg>
                            </button>
                        </form>
                    </td>
                    <td>
                        ...
                        @if ($ad->Status->title == 'blocked')
                            <form class="formColumn" action="{{ route('adUpdate', $ad['id']) }}" method="post">
                                @method('PUT')
                                @csrf
                                <input class="formChange form-sort__change" type="text" name="blocked_message" value="{{ $ad['blocked_message'] }}">
                                <Button class="btn btn-danger bthChange admin-table__bth-change">
                                   <p>Обновить</p> bbbbb
                                    <svg class="svgAdmin admin-table__bth-svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.com/svgjs" version="1.1" width="512" height="512" x="0" y="0" viewBox="0 0 465.822 465.822" style="enable-background:new 0 0 512 512" xml:space="preserve"><g>
                    ...
                    <td class="d-flex flex-row">
                        <form action="{{ route('adDestroy', $ad['id']) }}" method="post">
                            @method('DELETE')
                            @csrf
                            <Button class="btn btn-danger admin-table__bth-delete" @if(Auth::user()->isModerator()) disabled @endif>
                                <svg class="admin-table__bth-svg" width="48" height="49" viewBox="0 0 48 49" fill="none" xmlns="http://www.w3.org/2000/svg">
