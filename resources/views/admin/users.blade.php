            ...
            @foreach ($users as $user)
                <tr @if ($user->status_id == 2) class='border-danger' @endif>
                    <form class="form-group admin-table__column" action="{{ route('user.update', $user['id']) }}"
                        ...
                        @csrf
                        @method('PUT')
                        <td class=""><a href="{{ route('user.show', $user['id']) }}">{{ $user['id'] }}</a></td>
                        <td><input class="form-control" type="text" name="name" value="{{ $user['name'] }}">
                            @if($user->isOnline())
                                <div><span style="color:green">В сети c {{$user['login_time']}} </span></div>
                            @else
                                <div><span style="color:red">Был в сети {{$user['logout_time']}}</span></div>
                            @endif
                        </td>
                        <td> {{ $user['email'] }} </td>
                        <td>
