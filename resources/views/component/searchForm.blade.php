<div class="container">
    <form class="d-flex flex-column w-100" action="{{ route('search') }}" method="POST">
        ...
        <div class="form-group">
            <input type="text" class="form-control-input" @if(isset($searchWord)) value="{{$searchWord}}"
                   @endif placeholder="Наименование" name="name">
        </div>
            ...
            <select class="form-control-input" name="city">
                ...
                @foreach($cities as $city)
                    <option value="{{ $city['id'] }}"
                            @if(isset($city_selected) && $city_selected==$city['id']) selected @endif>{{ $city['name'] }}</option>
                @endforeach
            </select>
        </div>
                ...
                <select class="form-control-input" name="category">
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category['id'] }}"
                                @if(isset($categorise_selected) && $categorise_selected==$category['id']) selected @endif>{{ $category['title'] }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group w-50">
                <select class="form-control-input" name="barter_type">
                    ...
                    @foreach($barter_types as $barter_type)
                        <option value="{{ $barter_type[0] }}"
                                @if(isset($barter_type_selected) && $barter_type_selected == $barter_type[0]) selected
                            @endif>{{ $barter_type[1] }}</option>
                    @endforeach
                </select>
            </div>
        </div>
